...
public class polynomial implements IPolynomialSolver{
	...
	public int[][] add(char poly1, char poly2) {
		...
		ILinkedList list1 = set_list(poly1);
		ILinkedList list2 = set_list(poly2);
		int len1=list1.size() ; 
		int len2=list2.size();
		...
		while(i<len1 && j<len2)
		{
			Pair a = (Pair) list1.get(i);
			Pair b = (Pair) list2.get(j);
			if(a.exponent > b.exponent && a.coeff!=0)
			{
				listR.add(a);
				i++;
			...
			else if(a.exponent < b.exponent &&b.coeff!=0)
			{
				b.coeff*=sign;
				listR.add(b);
				j++;
